...
if ( ! class_exists( 'ReduxFramework_number' ) ) {
    class ReduxFramework_number {
        ...
        function render() {
            ...
            if(!isset($this->field['step']) || !$this->field['step'] || empty($this->field['step']))
                $this->field['step'] = 1;
            if ( isset( $this->field['options'] ) && ! empty( $this->field['options'] ) ) {
                ...
                foreach ( $this->field['options'] as $k => $v ) {
                    ...
                    echo '<input ' . $qtip_title . $qtip_text . 'type="number" max="'.$this->field['max'].'" min="'.$this->field['min'].'" step="'.$this->field['step'].'" id="' . $this->field['id'] . '-text-' . $k . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '[' . $k . ']' . '" ' . $placeholder . 'value="' . esc_attr( $this->value[ $k ] ) . '" class="regular-text regular-number ' . $this->field['class'] . '"' . $readonly . ' /><br />';
                    echo '</div>';
                }
                //foreach
            } else {
                $placeholder = ( isset( $this->field['placeholder'] ) && ! is_array( $this->field['placeholder'] ) ) ? ' placeholder="' . esc_attr( $this->field['placeholder'] ) . '" ' : '';
                echo '<input ' . $qtip_title . $qtip_text . 'type="number" max="'.$this->field['max'].'" min="'.$this->field['min'].'" step="'.$this->field['step'].'" id="' . $this->field['id'] . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '" ' . $placeholder . 'value="' . esc_attr( $this->value ) . '" class="regular-text regular-number ' . $this->field['class'] . '"' . $readonly . ' />';
            }
        }
        ...
        function enqueue() {
            ...
            wp_enqueue_script('number_format', ReduxFramework::$_url . 'inc/fields/number/field_number.js', array('jquery'), time(), true);
        }
    }
}